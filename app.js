...
function formToObject() {
  const data = {};
  Array.from(el.form.elements).forEach((n) => { if (!n.name || n.type === "button") return; data[n.name] = n.type === "file" ? ((n.files && n.files[0]) ? n.files[0].name : "") : n.type === "checkbox" ? n.checked : n.value; });
...
function saveProgress() {
  const ok = safeStorageSet("cddDemoProgress", JSON.stringify({ currentStep, simulatedMitId, data: formToObject() }));
  if (ok) alert(t("saved"));
}
function loadProgress() {
  const raw = safeStorageGet("cddDemoProgress"); if (!raw) return addUbo();
  try {
...
function submitApplication(e) {
  ...
  cases.unshift(c); safeStorageRemove("cddDemoProgress"); renderCases(); alert(t("submitted"));
